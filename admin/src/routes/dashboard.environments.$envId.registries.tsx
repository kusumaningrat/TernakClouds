...
function BoundImageBrowserPanel({
  ...
  const {
    ...
    error: reposError,
  } = useBoundRepos(workspaceSlug, envSlug, binding?.registry_id ?? "", !!binding);
  const {
    data: tags,
    ...
    return (
      <div className="rounded-xl border border-dashed border-border bg-card/50 flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Click <span className="font-medium text-foreground">Browse images</span> on a registry
          card to explore its accessible repositories.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      {selectedRepo ? (
                ...
                return (
                  <tr key={tag.name} className="hover:bg-accent/40 transition">
                    <td className="px-4 py-3">
                      <span className="font-mono text-xs bg-secondary border border-border px-1.5 py-0.5 rounded">
                        {tag.name}
                      </span>
                    </td>
                    <td className="px-4 py-3">
        ...
        <div className="text-sm text-muted-foreground py-10 text-center">
          No repositories accessible in this environment.
        </div>
      ) : (
        (() => {
