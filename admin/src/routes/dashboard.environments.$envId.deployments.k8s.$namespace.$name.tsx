...
import { useK8sDeploymentDetail, useK8sPods, useScaleK8sDeployment } from "@/lib/queries";
...
function PodsSection({
  ...
  const filtered = pods.filter((pod) =>
    Object.entries(selector).every(([k, v]) => pod.name.includes(k) || pod.name.includes(v)),
  );
          ...
          {filtered.map((pod) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-xs text-muted-foreground">{pod.ready}</td>
                <td className="px-3 py-2.5">
                  <span
...
function K8sDeploymentDetailPage() {
  ...
  const { data: detail, isLoading, error } = useK8sDeploymentDetail(slug, envId, namespace, name);

  const { data: pods = [], isLoading: podsLoading } = useK8sPods(slug, envId, namespace);
  ...
  const isHealthy = detail != null && detail.ready >= detail.desired && detail.desired > 0;
          ...
          <>
            ...
            <section className="space-y-3">
              ...
              <div className="rounded-lg border border-border bg-card p-4 flex items-center gap-4">
                ...
                {scaleDeployment.isSuccess && <span className="text-xs text-success">Scaled!</span>}
                    ...
                    <tbody className="bg-background">
                      {detail.containers.map((c) => (
                        <tr key={c.name} className="border-b border-border last:border-0">
                          <td className="px-3 py-2.5 font-medium text-sm">{c.name}</td>
