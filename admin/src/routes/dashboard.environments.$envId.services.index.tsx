...
import { Loader2, AlertTriangle, Server, Layers, FlaskConical, RefreshCw } from "lucide-react";
...
function K8sDeploymentCard({ dep, envId }: { dep: K8sDeploymentStub; envId: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-4 hover:shadow-sm transition">
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        <span>
          <span className="font-mono font-medium text-foreground">{dep.ready}</span>/{dep.desired}{" "}
          ready
        </span>
        <span>
          <span className="font-mono font-medium text-foreground">{dep.upToDate}</span> up-to-date
        </span>
        <span>
          <span className="font-mono font-medium text-foreground">{dep.available}</span> available
        </span>
      </div>
...
function K8sServiceCatalogView({ slug, envId }: { slug: string; envId: string }) {
      ...
      {!isLoading && !error && deployments.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {deployments.map((dep) => (
              <K8sDeploymentCard key={`${dep.namespace}/${dep.name}`} dep={dep} envId={envId} />
            ))}
          </div>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="bg-background">
                {services.map((svc: K8sServiceStub) => (
                  ...
                  >
                    <td className="px-3 py-2.5 font-medium text-sm font-mono">{svc.name}</td>
...
function NomadServiceCatalogView({ slug, envId }: { slug: string; envId: string }) {
  const [namespace, setNamespace] = useState("default");
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <div className="rounded-lg border border-border bg-secondary/30 px-4 py-2.5 text-xs text-muted-foreground leading-relaxed">
        Services are currently sourced from Nomad jobs. A first-class service registry with
        independent lifecycle management is coming soon — this view reflects the live runtime state.
      </div>
      ...
      {error && !isLoading && (
        <div className="flex items-center gap-2 text-sm text-destructive py-4">
          <AlertTriangle className="size-4" /> Failed to fetch services from Nomad
        </div>
      )}
      ...
      {!isLoading && !error && serviceJobs.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {serviceJobs.map((job) => (
              <ServiceCard key={job.ID} job={job} envId={envId} namespace={namespace} />
            ))}
          </div>
        </div>
      )}
      ...
      {!isLoading && !error && serviceJobs.length === 0 && otherJobs.length > 0 && (
        <div className="rounded-xl border border-dashed border-border bg-card/50 flex flex-col items-center justify-center py-12 text-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            No <span className="font-mono">service</span> type jobs found in this namespace.
          </p>
...
function EnvServicesPage() {
  ...
  return (
    <>
      ...
      {!capLoading && hasK8s && slug && <K8sServiceCatalogView slug={slug} envId={envId} />}
