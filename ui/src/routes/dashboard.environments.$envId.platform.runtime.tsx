...
function DockerContainerStateDot({ state }: { state: string }) {
  const cls =
    state === "running"
      ? "bg-success"
      : state === "exited"
...
function DockerContainerCard({ container }: { container: DockerContainerStub }) {
  const stateColor =
  ...
  return (
    <div className="rounded-lg border border-border bg-background/50 p-4 flex flex-col gap-3">
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-sm truncate">{container.name}</div>
          <div className="flex items-center gap-1.5 mt-0.5">
            <DockerContainerStateDot state={container.state} />
            <span className={`text-xs capitalize ${stateColor}`}>{container.state}</span>
          </div>
        </div>
      </div>
      <div className="space-y-0.5 text-xs text-muted-foreground font-mono">
        <div className="truncate">{container.image}</div>
        {container.status && <div className="truncate">{container.status}</div>}
      </div>
...
function DockerContainersView({ slug, envId }: { slug: string; envId: string }) {
  const { data: containers = [], isLoading, error, refetch } = useDockerContainers(slug, envId);
  ...
  return (
    <div className="space-y-4">
      ...
      {!isLoading && !error && containers.length === 0 && (
        <p className="text-sm text-muted-foreground py-4">No containers found.</p>
      )}
      {containers.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {containers.map((c) => (
            <DockerContainerCard key={c.id} container={c} />
          ))}
        </div>
      )}
    </div>
...
function DockerRuntimeDashboard({ slug, envId }: { slug: string; envId: string }) {
  return (
    <section className="px-6 pb-8">
      <div className="border-t border-border pt-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="font-semibold text-sm">Docker Engine</h2>
            <p className="text-xs text-muted-foreground">Containers and runtime state</p>
          </div>
        </div>
        <DockerContainersView slug={slug} envId={envId} />
      </div>
    </section>
  );
}
  ...
  return (
    <CapabilityPage
      ...
      endpointPlaceholders={{
        ...
        docker: "http://docker-host:2375",
      }}
      extraContent={
        slug && (hasNomad || hasK8s || hasDocker) ? (
          <>
            {hasNomad && <NomadRuntimeDashboard slug={slug} envId={envId} />}
            {hasK8s && <K8sRuntimeDashboard slug={slug} envId={envId} />}
            {hasDocker && <DockerRuntimeDashboard slug={slug} envId={envId} />}
          </>
        ) : null
      }
    />
