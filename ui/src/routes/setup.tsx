...
function SetupPage() {
  ...
  useEffect(() => {
    if (workspacesLoading || (slug && envsLoading) || capsLoading || step !== null) return;
  ...
  if (workspacesLoading || (slug && envsLoading) || capsLoading || step === null) {
    return (
    ...
    <Card>
      ...
      <div className="space-y-4">
        ...
        <div className="p-3 rounded-lg bg-secondary/50 text-xs text-muted-foreground">
          💡 You'll create environments and connect your runtime in the next steps.
        </div>
      </div>
...
function RuntimeStep({
  ...
  if (existing) {
    const iconMap: Record<string, string> = { kubernetes: "☸", nomad: "📦", docker: "🐳" };
...
function ServiceStep({
  ...
  const submit = async () => {
    ...
    const item = catalog?.find((c) => c.name === selectedService);
    setError("");
    try {
      await deployService.mutateAsync({
        ...
        exposed_port: item?.default_container_port ?? 80,
      });
      onDone();
    } catch (e: unknown) {
      setError(
    ...
    <Card>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          {error && <p className="text-sm text-destructive">{error}</p>}
          <div className="p-3 rounded-lg bg-secondary/50 text-xs text-muted-foreground">
            💡 Requires a connected runtime. If you skipped Step 3, deploy from the dashboard later.
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 mb-8">
        {items.map(({ step, label }) => (
          ...
          >
            ...
            {!completed.includes(step) && (
              <span className="text-[10px] font-mono text-muted-foreground">SKIPPED</span>
            )}
          </div>
        ))}
      </div>
