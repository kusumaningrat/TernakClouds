...
function K8sServiceDetailPage() {
  ...
  const { data: detail, isLoading, error } = useK8sServiceDetail(
    slug,
    envId,
    namespace,
    name
  ...
  const typeCls =
    SERVICE_TYPE_COLOR[detail?.type ?? ""] ??
  ...
  return (
    <>
      <DashboardTopbar
        title={name}
        subtitle={`K8s Service · namespace: ${namespace}`}
      />
        ...
        {!isLoading && detail && (
          <>
            ...
            <div className="flex items-center gap-3">
              <span
                className={`text-xs font-medium px-2 py-1 rounded border ${typeCls}`}
              >
                {detail.type}
              </span>
            </div>
            ...
            <section className="space-y-3">
              ...
              <div className="rounded-lg border border-border bg-card p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <span
                    className={`text-xs font-medium px-1.5 py-0.5 rounded border ${typeCls}`}
                  >
                    {detail.type}
                  </span>
                </div>
