...
import {
  useNomadJob, useNomadAllocations, useNomadAllocation,
  useNomadDeployments, useStopJob, useStartJob,
} from "@/lib/queries";
import { useState, useEffect, useRef } from "react";
import {
  Loader2, AlertTriangle, ArrowLeft, Cpu, MemoryStick,
  Square, Play, AlertCircle, Terminal, X,
} from "lucide-react";
...
const ALLOC_DOT: Record<string, string> = {
  running: "bg-success", pending: "bg-yellow-500", complete: "bg-blue-500",
  failed: "bg-destructive", lost: "bg-muted-foreground", unknown: "bg-muted-foreground",
};
const ALLOC_TEXT: Record<string, string> = {
  running: "text-success", pending: "text-yellow-500", complete: "text-blue-500",
  failed: "text-destructive", lost: "text-muted-foreground", unknown: "text-muted-foreground",
};
...
const JOB_STATUS_DOT: Record<string, string> = {
  running: "bg-success", pending: "bg-yellow-500", dead: "bg-muted-foreground",
};
const JOB_STATUS_TEXT: Record<string, string> = {
  running: "text-success", pending: "text-yellow-500", dead: "text-muted-foreground",
};

// ─── Stop confirmation modal ──────────────────────────────────────────────────

function StopModal({ jobName, isPending, onConfirm, onCancel }: {
  jobName: string; isPending: boolean;
  onConfirm: (purge: boolean) => void; onCancel: () => void;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="glass rounded-xl border border-border shadow-2xl w-full max-w-sm mx-4 p-6 space-y-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              <span className="font-mono font-medium text-foreground">{jobName}</span>{" "}
              will be deregistered and all running allocations stopped.
            </p>
          </div>
        </div>
        <label className="flex items-start gap-2.5 cursor-pointer group">
          ...
          <div>
            <span className="text-sm font-medium group-hover:text-foreground transition">Purge job</span>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={() => onConfirm(purge)} disabled={isPending}
...
interface LogStreamParams {
  ...
  task: string; logType: "stdout" | "stderr"; follow: boolean; enabled: boolean;
}
...
function LogsDrawer({ jobID, jobName, slug, envId, namespace, onClose }: {
  jobID: string; jobName: string; slug: string; envId: string;
  namespace: string; onClose: () => void;
}) {
  ...
  const { data: allocs = [], isLoading: allocsLoading } = useNomadAllocations(slug, envId, jobID, namespace);
  ...
  const { lines, connected, streamError, clear } = useLogStream({
    slug, envSlug: envId, allocID: selectedAllocID ?? "",
    task: selectedTask, logType, follow,
    enabled: !!selectedAllocID && !!selectedTask && rightTab === "logs",
  });

  const { data: allocDetail, isLoading: allocDetailLoading } = useNomadAllocation(
    slug, envId, selectedAllocID ?? "", !!selectedAllocID && rightTab === "events",
  );
  ...
  const handleSelectAlloc = (id: string) => {
    setSelectedAllocID(id); setSelectedTask(""); atBottomRef.current = true;
  };
        ...
        <div className="flex flex-1 overflow-hidden">
          <aside className="w-52 shrink-0 border-r border-border flex flex-col overflow-hidden">
            <div className="px-3 py-2 border-b border-border shrink-0">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Allocations</p>
            </div>
            <div className="flex-1 overflow-y-auto">
              ...
              {allocs.map((alloc: NomadAllocationStub) => {
                ...
                return (
                  <button key={alloc.ID} onClick={() => handleSelectAlloc(alloc.ID)}
                    ...
                    <div className="text-[10px] text-muted-foreground truncate mt-0.5 pl-3">{alloc.NodeName}</div>
                    <div className="text-[10px] font-mono text-muted-foreground truncate pl-3">{alloc.TaskGroup}</div>
                  </button>
                );
              })}
            </div>
              ...
              <>
                ...
                {rightTab === "events" && (
                  <div className="flex-1 overflow-y-auto p-3 space-y-1">
                    ...
                    {!allocDetailLoading && selectedTask && allocDetail?.TaskStates?.[selectedTask]?.Events?.length
                      ? [...allocDetail.TaskStates[selectedTask].Events].reverse().map((ev, i) => (
                          <div key={i} className={`rounded-lg border px-3 py-2 space-y-0.5 ${ev.FailsTask ? "border-destructive/40 bg-destructive/5" : "border-border bg-background"}`}>
                            <div className="flex items-center gap-2">
                              {ev.FailsTask && <AlertTriangle className="size-3 text-destructive shrink-0" />}
                              <span className={`text-[11px] font-semibold ${ev.FailsTask ? "text-destructive" : "text-foreground"}`}>{ev.Type}</span>
                              <span className="text-[10px] text-muted-foreground ml-auto">{ev.Time ? formatTime(ev.Time / 1_000_000) : ""}</span>
                            </div>
                            {ev.DisplayMessage && <p className="text-xs text-muted-foreground pl-0.5">{ev.DisplayMessage}</p>}
                            {ev.Details && Object.keys(ev.Details).length > 0 && (
                              <div className="flex flex-wrap gap-1 pt-0.5">
                                {Object.entries(ev.Details).map(([k, v]) => (
                                  <span key={k} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">
                      ...
                      : !allocDetailLoading && (
                          <p className="text-xs text-muted-foreground py-4">
                            {selectedTask ? "No events for this task." : "Select a task to view events."}
                          </p>
                        )}
                  </div>
                )}
                  ...
                  <>
                    <div className="flex items-center gap-2 px-3 py-2 border-b border-border shrink-0 flex-wrap">
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer text-xs text-muted-foreground select-none">
                        <input type="checkbox" checked={follow} onChange={(e) => setFollow(e.target.checked)} className="accent-primary cursor-pointer" />
                        Follow
                      </label>
                      <button onClick={() => { clear(); atBottomRef.current = true; }}
                        className="text-xs px-2 py-1 rounded border border-border hover:bg-accent transition text-muted-foreground">
                        Clear
                      </button>
                      <div className="ml-auto flex items-center gap-1.5 shrink-0">
                        {connected ? (
                          <><span className="size-1.5 rounded-full bg-success animate-pulse" /><span className="text-[11px] text-success font-medium">live</span></>
                        ) : streamError ? (
                          <span className="text-[11px] text-destructive truncate max-w-[140px]" title={streamError}>{streamError}</span>
                        ) : (
                          <><span className="size-1.5 rounded-full bg-muted-foreground" /><span className="text-[11px] text-muted-foreground">disconnected</span></>
                        )}
                      </div>
                    </div>
                    <div ref={termRef} onScroll={handleTermScroll}
                      className="flex-1 overflow-y-auto bg-[#0d0d0d] p-3 font-mono text-xs text-green-300 leading-relaxed">
                      {lines.length === 0 ? (
                        <span className="text-zinc-600 italic">{connected ? "Waiting for logs…" : "Connecting…"}</span>
                      ) : (
    ...
    <div className="space-y-5">
      <div className="rounded-lg border border-border bg-card p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <span className="text-[11px] font-mono px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">{detail.Type}</span>
        </div>
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Namespace</p>
        ...
        {(detail.Datacenters?.length ?? 0) > 0 && (
          <div className="col-span-2">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Datacenters</p>
            <div className="flex flex-wrap gap-1">
              {detail.Datacenters.map((dc) => (
                <span key={dc} className="text-[11px] font-mono px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">{dc}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {detail.ModifyTime && (
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Last Modified</p>
      ...
      {detail.TaskGroups && detail.TaskGroups.length > 0 && (
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Task Groups</p>
            ...
            {detail.TaskGroups.map((tg) => (
              <div key={tg.Name} className="rounded-lg border border-border bg-card p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">×{tg.Count}</span>
                </div>
                {tg.Tasks && tg.Tasks.length > 0 && (
                  <div className="space-y-1.5">
                    {tg.Tasks.map((task) => (
                      <div key={task.Name} className="rounded-md bg-secondary/60 px-2.5 py-2 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium">{task.Name}</span>
                          <span className="text-[10px] font-mono text-muted-foreground px-1 py-px rounded bg-background border border-border">{task.Driver}</span>
                        </div>
                        {task.Resources && (
                          <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
                            {task.Resources.CPU != null && <span className="flex items-center gap-1"><Cpu className="size-3" />{task.Resources.CPU} MHz</span>}
                            {task.Resources.MemoryMB != null && <span className="flex items-center gap-1"><MemoryStick className="size-3" />{task.Resources.MemoryMB} MB</span>}
                          </div>
                        )}
                      </div>
                    ))}
      ...
      {hasMeta && (
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Metadata</p>
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(detail.Meta!).map(([k, v]) => (
              <span key={k} className="text-[11px] font-mono px-2 py-0.5 rounded bg-secondary border border-border text-muted-foreground">
                {k}: <span className="text-foreground">{v}</span>
              </span>
            ))}
          </div>
...
function AllocationsSection({ allocs, isLoading }: { allocs: NomadAllocationStub[]; isLoading: boolean }) {
  if (isLoading) return <div className="flex items-center gap-2 text-sm text-muted-foreground py-4"><Loader2 className="size-4 animate-spin" /> Loading allocations…</div>;
  if (allocs.length === 0) return <p className="text-sm text-muted-foreground py-4">No allocations found.</p>;
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="bg-background">
          {allocs.map((alloc) => {
            ...
            return (
              <tr key={alloc.ID} className="border-b border-border last:border-0 hover:bg-accent/20 transition">
                <td className="px-3 py-2.5 font-mono text-xs">{alloc.ID.slice(0, 8)}</td>
                <td className="px-3 py-2.5">
                  <div className="flex items-center gap-1.5">
                    <span className={`size-1.5 rounded-full shrink-0 ${dot}`} />
                    <span className={`text-xs capitalize font-medium ${text}`}>{alloc.ClientStatus}</span>
                  </div>
                </td>
                <td className="px-3 py-2.5 text-xs font-mono text-muted-foreground">{alloc.TaskGroup}</td>
                <td className="px-3 py-2.5 text-xs text-muted-foreground">{alloc.NodeName || "—"}</td>
                <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">{formatTime(alloc.CreateTime)}</td>
              </tr>
            );
          })}
        </tbody>
...
function DeploymentHistorySection({ deployments, isLoading, jobType }: {
  deployments: NomadDeploymentStub[]; isLoading: boolean; jobType: string;
}) {
  if (jobType !== "service") {
    return <p className="text-sm text-muted-foreground py-4">Deployment history is only tracked for <span className="font-mono">service</span> type jobs.</p>;
  }
  if (isLoading) return <div className="flex items-center gap-2 text-sm text-muted-foreground py-4"><Loader2 className="size-4 animate-spin" /> Loading deployments…</div>;
  if (deployments.length === 0) return <p className="text-sm text-muted-foreground py-4">No deployments recorded.</p>;
  ...
  return (
    <div className="space-y-2">
      {sorted.map((dep: NomadDeploymentStub) => {
        const st = DEPLOY_STATUS[dep.Status] ?? { dot: "bg-muted-foreground", text: "text-muted-foreground" };
        return (
          <div key={dep.ID} className="rounded-lg border border-border bg-card p-3 space-y-2">
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[11px] font-semibold capitalize ${st.text}`}>{dep.Status}</span>
              </div>
              <span className="text-[11px] font-mono px-1.5 py-0.5 rounded bg-secondary border border-border text-muted-foreground">v{dep.JobVersion}</span>
              {dep.StatusDescription && <span className="text-[11px] text-muted-foreground">{dep.StatusDescription}</span>}
              <span className="text-[11px] font-mono text-muted-foreground ml-auto">{formatTime(dep.ModifyTime)}</span>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-1 border-t border-border">
                {Object.entries(dep.TaskGroups).map(([tg, tgs]) => (
                  <div key={tg} className="flex items-center gap-2 text-[11px]">
                    ...
                    <span className="text-muted-foreground shrink-0">{tgs.PlacedAllocs}/{tgs.DesiredTotal} placed</span>
                    <span className="text-success shrink-0">{tgs.HealthyAllocs} healthy</span>
                    {tgs.UnhealthyAllocs > 0 && <span className="text-destructive shrink-0">{tgs.UnhealthyAllocs} unhealthy</span>}
                  </div>
                ))}
              </div>
            )}
...
function JobDetailPage() {
  ...
  const { data: detail, isLoading: jobLoading, error: jobError } = useNomadJob(slug, envId, jobId, namespace);
  const { data: allocs = [], isLoading: allocsLoading } = useNomadAllocations(slug, envId, jobId, namespace);
  const { data: deployments = [], isLoading: deploymentsLoading } = useNomadDeployments(slug, envId, jobId, namespace);
  ...
  return (
    <>
      ...
      <main className="p-6 space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          {detail && (
            <div className="flex items-center gap-2">
              ...
              {canStop && (
                <button onClick={() => setShowStop(true)} disabled={stopJob.isPending}
                  ...
                  {stopJob.isPending ? <Loader2 className="size-3.5 animate-spin" /> : <Square className="size-3.5" />} Stop
                </button>
              )}
              {canStart && (
                <button onClick={() => void startJob.mutateAsync({ slug, envSlug: envId, jobID: jobId, namespace })} disabled={startJob.isPending}
                  ...
                  {startJob.isPending ? <Loader2 className="size-3.5 animate-spin" /> : <Play className="size-3.5" />} Start
                </button>
              )}
            </div>
          )}
        ...
        {!jobLoading && detail && (
          <section className="space-y-3">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Deployment History</p>
            <DeploymentHistorySection deployments={deployments} isLoading={deploymentsLoading} jobType={detail.Type} />
          </section>
        )}
      </main>
