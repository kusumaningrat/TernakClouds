...
function NomadNodeStatusDot({ status }: { status: string }) {
  const cls = status === "ready" ? "bg-success" : status === "down" ? "bg-destructive" : "bg-yellow-500";
...
function NomadNodeCard({ node }: { node: NomadNodeStub }) {
  const healthyDrivers = Object.entries(node.Drivers ?? {})
    ...
    .map(([name]) => name);
  const statusColor = node.Status === "ready" ? "text-success" : node.Status === "down" ? "text-destructive" : "text-yellow-500";
  return (
    <div className="rounded-lg border border-border bg-background/50 p-4 flex flex-col gap-3">
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            {node.Drain && (
              <span className="text-[10px] px-1 py-px rounded bg-yellow-500/15 text-yellow-600 border border-yellow-500/30">draining</span>
            )}
          </div>
        </div>
      </div>
      <div className="space-y-0.5 text-xs text-muted-foreground font-mono">
        ...
        {node.Datacenter && <div>{node.Datacenter}{node.NodeClass ? ` · ${node.NodeClass}` : ""}</div>}
      </div>
      {healthyDrivers.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {healthyDrivers.map((d) => (
            <span key={d} className="text-[10px] px-1.5 py-0.5 rounded bg-secondary border border-border font-mono">{d}</span>
          ))}
        </div>
      )}
    </div>
...
function NomadNodesView({ slug, envId }: { slug: string; envId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && <div className="flex items-center gap-2 text-sm text-muted-foreground py-4"><Loader2 className="size-4 animate-spin" /> Loading nodes…</div>}
      {error && !isLoading && <div className="flex items-center gap-2 text-sm text-destructive py-2"><AlertTriangle className="size-4" /> Failed to reach Nomad cluster</div>}
      {!isLoading && !error && nodes.length === 0 && <p className="text-sm text-muted-foreground py-4">No nodes registered.</p>}
      {nodes.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {nodes.map((node) => <NomadNodeCard key={node.ID} node={node} />)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="rounded-lg border border-border bg-background/50 p-4 flex flex-col gap-3">
      ...
      {node.roles.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {node.roles.map((r) => (
            <span key={r} className="text-[10px] px-1.5 py-0.5 rounded bg-secondary border border-border font-mono">{r}</span>
          ))}
        </div>
      )}
    </div>
...
function K8sNodesView({ slug, envId }: { slug: string; envId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && <div className="flex items-center gap-2 text-sm text-muted-foreground py-4"><Loader2 className="size-4 animate-spin" /> Loading nodes…</div>}
      {error && !isLoading && <div className="flex items-center gap-2 text-sm text-destructive py-2"><AlertTriangle className="size-4" /> Failed to reach Kubernetes cluster</div>}
      {!isLoading && !error && nodes.length === 0 && <p className="text-sm text-muted-foreground py-4">No nodes registered.</p>}
      {nodes.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {nodes.map((node) => <K8sNodeCard key={node.name} node={node} />)}
        </div>
      )}
    </div>
  );
