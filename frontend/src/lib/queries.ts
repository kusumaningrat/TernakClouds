...
export const workspaceKeys = {
  ...
  integrations: (slug: string, envSlug: string) => ['workspaces', slug, 'environments', envSlug, 'integrations'] as const,
};
...
export function useApproveAccessRequest() {
  const queryClient = useQueryClient();
  return useMutation<{ message: string }, ApiError, { id: string; input?: ApproveAccessRequestInput }>({
...
export function useRemoveMember() {
  ...
  return useMutation<{ message: string }, ApiError, { slug: string; userId: string }>({
    mutationFn: ({ slug, userId }) =>
      api.delete(`/api/v1/workspaces/${slug}/members/${userId}`),
    onSuccess: (_, { slug }) => {
      void queryClient.invalidateQueries({ queryKey: workspaceKeys.members(slug) });
    },
  });
...
export function useCreateEnvironment() {
  const queryClient = useQueryClient();
  return useMutation<WorkspaceEnvironment, ApiError, { slug: string; input: CreateEnvironmentInput }>({
    mutationFn: ({ slug, input }) =>
      api.post(`/api/v1/workspaces/${slug}/environments`, input),
    onSuccess: (_, { slug }) => {
...
export function useAddIntegration() {
  const queryClient = useQueryClient();
  return useMutation<Integration, ApiError, { slug: string; envSlug: string; input: AddIntegrationInput }>({
    mutationFn: ({ slug, envSlug, input }) =>
...
export function useSecretValue(slug: string, envSlug: string, id: string, enabled = true) {
  return useQuery<SecretValueResponse, ApiError>({
    ...
    queryFn:  () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/secrets/${id}/value`),
...
export function useCreateSecretGrant() {
  const queryClient = useQueryClient();
  return useMutation<SecretGrantAdminView, ApiError, { slug: string; envSlug: string; input: CreateSecretGrantInput }>({
    mutationFn: ({ slug, envSlug, input }) =>
...
export function useUpdateSecretGrant() {
  const queryClient = useQueryClient();
  return useMutation<SecretGrantAdminView, ApiError, { slug: string; envSlug: string; id: string; input: UpdateSecretGrantInput }>({
...
export function useWriteSecretValue() {
  const queryClient = useQueryClient();
  return useMutation<void, ApiError, { slug: string; envSlug: string; id: string; path?: string; data: Record<string, string> }>({
    mutationFn: ({ slug, envSlug, id, path = "", data }) =>
      api.put(`/api/v1/workspaces/${slug}/environments/${envSlug}/secrets/${id}/value`, { path, data }),
...
export const capabilityKeys = {
  all: (slug: string, envSlug: string) => ['workspaces', slug, 'environments', envSlug, 'capabilities'] as const,
  detail: (slug: string, envSlug: string, cap: string) => ['workspaces', slug, 'environments', envSlug, 'capabilities', cap] as const,
...
export function useCapability(slug: string, envSlug: string, cap: string) {
  return useQuery<CapabilityStatusResponse, ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/capabilities/${cap}`),
...
export function useCapabilityProviders(slug: string, envSlug: string, cap: string) {
  return useQuery<CapabilityProvider[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/capabilities/${cap}/providers`),
    enabled: !!slug && !!envSlug && !!cap,
  });
}
...
export function useBindProvider() {
  const queryClient = useQueryClient();
  return useMutation<CapabilityStatusResponse, ApiError, { slug: string; envSlug: string; cap: string; input: BindProviderInput }>({
    mutationFn: ({ slug, envSlug, cap, input }) =>
      api.post(`/api/v1/workspaces/${slug}/environments/${envSlug}/capabilities/${cap}/provider`, input),
...
export function useUpdateProvider() {
  const queryClient = useQueryClient();
  return useMutation<CapabilityStatusResponse, ApiError, { slug: string; envSlug: string; cap: string; providerID: string; input: UpdateProviderInput }>({
    mutationFn: ({ slug, envSlug, cap, providerID, input }) =>
      api.put(`/api/v1/workspaces/${slug}/environments/${envSlug}/capabilities/${cap}/provider/${providerID}`, input),
    onSuccess: (_, { slug, envSlug, cap }) => {
      void queryClient.invalidateQueries({ queryKey: capabilityKeys.all(slug, envSlug) });
      void queryClient.invalidateQueries({ queryKey: capabilityKeys.detail(slug, envSlug, cap) });
    },
...
export function useUnbindProvider() {
  const queryClient = useQueryClient();
  return useMutation<{ message: string }, ApiError, { slug: string; envSlug: string; cap: string; providerID: string }>({
    mutationFn: ({ slug, envSlug, cap, providerID }) =>
      api.delete(`/api/v1/workspaces/${slug}/environments/${envSlug}/capabilities/${cap}/provider/${providerID}`),
    onSuccess: (_, { slug, envSlug, cap }) => {
      void queryClient.invalidateQueries({ queryKey: capabilityKeys.all(slug, envSlug) });
      void queryClient.invalidateQueries({ queryKey: capabilityKeys.detail(slug, envSlug, cap) });
    },
...
export function useNomadJobs(slug: string, envSlug: string, namespace: string, enabled = true) {
  return useQuery<NomadJobStub[], ApiError>({
    ...
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs?namespace=${encodeURIComponent(namespace)}`),
...
export function useNomadJob(slug: string, envSlug: string, jobID: string, namespace: string, enabled = true) {
  return useQuery<NomadJobDetail, ApiError>({
    queryKey: nomadKeys.job(slug, envSlug, namespace, jobID),
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}?namespace=${encodeURIComponent(namespace)}`),
...
export function useStopJob() {
  const queryClient = useQueryClient();
  return useMutation<NomadJobActionResponse, ApiError, { slug: string; envSlug: string; jobID: string; namespace: string; purge?: boolean }>({
    mutationFn: ({ slug, envSlug, jobID, namespace, purge = false }) =>
      api.post(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}/stop?namespace=${encodeURIComponent(namespace)}&purge=${purge}`, {}),
...
export function useNomadEvaluations(slug: string, envSlug: string, jobID: string, namespace: string, enabled = true) {
  return useQuery<NomadEvalStub[], ApiError>({
    queryKey: nomadKeys.evaluations(slug, envSlug, jobID, namespace),
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}/evaluations?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!jobID && !!namespace && enabled,
    staleTime: 15_000,
  });
}
...
export function useNomadDeployments(slug: string, envSlug: string, jobID: string, namespace: string, enabled = true) {
  return useQuery<NomadDeploymentStub[], ApiError>({
    queryKey: nomadKeys.deployments(slug, envSlug, jobID, namespace),
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}/deployments?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!jobID && !!namespace && enabled,
    staleTime: 15_000,
  });
}
...
export function useNomadAllocation(slug: string, envSlug: string, allocID: string, enabled = true) {
  return useQuery<NomadAllocationDetail, ApiError>({
    ...
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/allocations/${encodeURIComponent(allocID)}`),
...
export function useNomadAllocations(slug: string, envSlug: string, jobID: string, namespace: string, enabled = true) {
  return useQuery<NomadAllocationStub[], ApiError>({
    queryKey: nomadKeys.allocations(slug, envSlug, jobID, namespace),
    queryFn: () =>
      api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}/allocations?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!jobID && !!namespace && enabled,
    staleTime: 10_000,
  });
}
...
export function useStartJob() {
  const queryClient = useQueryClient();
  return useMutation<NomadJobActionResponse, ApiError, { slug: string; envSlug: string; jobID: string; namespace: string }>({
    mutationFn: ({ slug, envSlug, jobID, namespace }) =>
      api.post(`/api/v1/workspaces/${slug}/environments/${envSlug}/nomad/jobs/${encodeURIComponent(jobID)}/start?namespace=${encodeURIComponent(namespace)}`, {}),
    onSuccess: (_, { slug, envSlug, namespace }) => {
      void queryClient.invalidateQueries({ queryKey: nomadKeys.jobs(slug, envSlug, namespace) });
    },
  });
...
export const registryKeys = {
  ...
  repos: (slug: string, id: string) => ['workspaces', slug, 'registries', id, 'repositories'] as const,
  tags: (slug: string, id: string, repo: string) => ['workspaces', slug, 'registries', id, 'repositories', repo, 'tags'] as const,
  bindings: (slug: string, envSlug: string) => ['workspaces', slug, 'environments', envSlug, 'registries'] as const,
  boundRepos: (slug: string, envSlug: string, id: string) => ['workspaces', slug, 'environments', envSlug, 'registries', id, 'repositories'] as const,
  boundTags: (slug: string, envSlug: string, id: string, repo: string) => ['workspaces', slug, 'environments', envSlug, 'registries', id, 'repositories', repo, 'tags'] as const,
};
...
export function useRegistryTags(slug: string, id: string, repoName: string, enabled = true) {
  return useQuery<RegistryTag[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/registries/${id}/tags?repo=${encodeURIComponent(repoName)}`),
...
export function useCreateBinding(slug: string, envSlug: string) {
  ...
  return useMutation<RegistryBinding, ApiError, CreateRegistryBindingInput>({
    mutationFn: (input) => api.post(`/api/v1/workspaces/${slug}/environments/${envSlug}/registries`, input),
    onSuccess: () => {
...
export function useDeleteBinding(slug: string, envSlug: string) {
  const queryClient = useQueryClient();
  return useMutation<void, ApiError, string>({
    mutationFn: (bindingId) => api.delete(`/api/v1/workspaces/${slug}/environments/${envSlug}/registries/${bindingId}`),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: registryKeys.bindings(slug, envSlug) });
    },
  });
...
export function useBoundRepos(slug: string, envSlug: string, registryId: string, enabled = true) {
  return useQuery<RegistryRepo[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/registries/${registryId}/repositories`),
    enabled: !!slug && !!envSlug && !!registryId && enabled,
  });
}

// GET /api/v1/workspaces/:slug/environments/:envSlug/registries/:id/tags?repo=<repoName>
export function useBoundTags(slug: string, envSlug: string, registryId: string, repoName: string, enabled = true) {
  return useQuery<RegistryTag[], ApiError>({
    queryKey: registryKeys.boundTags(slug, envSlug, registryId, repoName),
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/registries/${registryId}/tags?repo=${encodeURIComponent(repoName)}`),
...
export function useK8sNamespaces(slug: string, envSlug: string, enabled = true) {
  return useQuery<K8sNamespaceStub[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/namespaces`),
    enabled: !!slug && !!envSlug && enabled,
  });
}

// GET /api/v1/workspaces/:slug/environments/:envSlug/kubernetes/deployments?namespace=...
export function useK8sDeployments(slug: string, envSlug: string, namespace: string, enabled = true) {
  return useQuery<K8sDeploymentStub[], ApiError>({
    queryKey: [...k8sKeys.deployments(slug, envSlug), namespace],
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/deployments?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!namespace && enabled,
  });
}
...
export function useK8sPods(slug: string, envSlug: string, namespace: string, enabled = true) {
  return useQuery<K8sPodStub[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/pods?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!namespace && enabled,
  });
}
...
export function useK8sServices(slug: string, envSlug: string, namespace: string, enabled = true) {
  return useQuery<K8sServiceStub[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/services?namespace=${encodeURIComponent(namespace)}`),
    enabled: !!slug && !!envSlug && !!namespace && enabled,
  });
}

// GET /api/v1/workspaces/:slug/environments/:envSlug/kubernetes/deployments/:namespace/:name
export function useK8sDeploymentDetail(slug: string, envSlug: string, namespace: string, name: string, enabled = true) {
  return useQuery<K8sDeploymentDetail, ApiError>({
    queryKey: k8sKeys.deploymentDetail(slug, envSlug, namespace, name),
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/deployments/${namespace}/${name}`),
...
export function useK8sPodDetail(slug: string, envSlug: string, namespace: string, name: string, enabled = true) {
  return useQuery<K8sPodDetail, ApiError>({
    queryKey: k8sKeys.podDetail(slug, envSlug, namespace, name),
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/pods/${namespace}/${name}`),
    enabled: !!slug && !!envSlug && !!namespace && !!name && enabled,
  });
}

// GET /api/v1/workspaces/:slug/environments/:envSlug/kubernetes/services/:namespace/:name
export function useK8sServiceDetail(slug: string, envSlug: string, namespace: string, name: string, enabled = true) {
  return useQuery<K8sServiceDetail, ApiError>({
    queryKey: k8sKeys.serviceDetail(slug, envSlug, namespace, name),
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/services/${namespace}/${name}`),
    enabled: !!slug && !!envSlug && !!namespace && !!name && enabled,
  });
}
...
export function useScaleK8sDeployment() {
  const queryClient = useQueryClient();
  return useMutation<{ message: string }, ApiError, { slug: string; envSlug: string; namespace: string; name: string; replicas: number }>({
    mutationFn: ({ slug, envSlug, namespace, name, replicas }) =>
      api.patch(`/api/v1/workspaces/${slug}/environments/${envSlug}/kubernetes/deployments/${namespace}/${name}/scale`, { replicas }),
    onSuccess: (_, { slug, envSlug, namespace, name }) => {
      void queryClient.invalidateQueries({ queryKey: k8sKeys.deploymentDetail(slug, envSlug, namespace, name) });
...
export function useServiceDeployments(slug: string, envSlug: string) {
  return useQuery<ServiceDeployment[], ApiError>({
    ...
    queryFn: () => api.get(`/api/v1/workspaces/${slug}/environments/${envSlug}/service-deployments`),
    enabled: !!slug && !!envSlug,
    staleTime: 15_000,
  });
}
